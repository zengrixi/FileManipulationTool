...
PictureBannerArrowWidget::PictureBannerArrowWidget(PictureBannerArrowWidget::ArrowType type, QWidget *parent)
    : QWidget(parent), _bMouseHover(false), _bMousePress(false), _type(type) {
...
void PictureBannerArrowWidget::mouseReleaseEvent(QMouseEvent *event) {
    if (event->button() == Qt::LeftButton) { _bMousePress = false; }
}
...
PictureBannerIndicator::PictureBannerIndicator(QWidget *parent) : QWidget(parent), _selected(false) { }
...
PictureBannerPage::PictureBannerPage(QWidget *parent) : QLabel(parent), _bActive(false) { }
...
void PictureBannerView::startAnimation(int direction) //开始动画
{
    ...
    QRect tmpRect = QRect(centerRect.x(), centerRect.y() + 12, centerRect.width(), centerRect.height() - 12);
    if (direction < 0) {
...
void PictureBannerView::setPagePixmap(PictureBannerPage *page, const QPixmap &pixmap) {
    if (page) { page->setPixmap(pixmap.scaled(_size, Qt::IgnoreAspectRatio, Qt::SmoothTransformation)); }
}
...
PictureBannerWidget::PictureBannerWidget(QWidget *parent) : QWidget(parent) {
    _view = nullptr;
    _indLayout = nullptr;
    _indicators.clear();
    _pixmaps.clear();
...
void PictureBannerWidget::addPages(const QList<QPixmap> &pixmaps) {
    foreach (QPixmap pixmap, pixmaps) { addPage(pixmap); }
}
...
void PictureBannerWidget::switchIndicator() {
    ...
    if (_currentIndex != -1) { _indicators.at(_currentIndex)->select(false); }
